...
{
	public static var SlicerVersion:String = '1';
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In the mods Slicer beat", null);
		#end
		...
		selection = new FlxSprite(0,0);
		selection.loadGraphic(Paths.image('menumod/selection/' + optionShit[curSelected]));
		selection.scrollFactor.set(0, 0);
		selection.setGraphicSize(Std.int(selection.width * 1));
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 64, 0, "Slicer Beat V" + SlicerVersion, 16);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			if (controls.UI_DOWN_P || controls.UI_UP_P){
				...
				FlxTween.tween(selection,{x: 600, alpha : 0},0.2,{ease:FlxEase.cubeIn,onComplete: function(twn:FlxTween)
					{
						selection.loadGraphic(Paths.image('menumod/selection/' + optionShit[curSelected]));
						FlxTween.tween(selection,{x: 0, alpha : 1},0.2,{ease:FlxEase.cubeOut});
					}
				});
			}
			if (controls.UI_UP_P)
